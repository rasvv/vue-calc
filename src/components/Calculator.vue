        ...
        <v-row class="calc__types">
          ...
          />
						...
						<v-row class="nuclids__top">
							<v-col cols="5" height="100%" class="d-flex justify-stretch flex-column calc__items">
								<v-radio-group v-model="favoriteNuclids" column
									...
									<v-radio label="Часто используемые" :value="1" class="calc__item"></v-radio>
								</v-radio-group>
								<v-text-field class="input" width="300px" label="Поиск" hide-details="auto" clearable
							...
							<v-col cols="6" class="nuclids__top-right">
								<v-radio-group v-model="showUda" column class="calc__items">
									<v-radio label="Удельная активность каждого нуклида" :value="0"
										class="calc__item"></v-radio>
									<!-- <v-radio
                    label="Общая удельная активность + %"
                    :value="1"
                    class="calc__item"
                  ></v-radio> -->
									<v-radio label="Суммарная активность + масса + %" :value="1"
										class="calc__item"></v-radio>
								</v-radio-group>
						...
						<v-row class="nuclids__bottom">
							<v-col cols="5">
									<v-list height="40vh" class="ml-4" dense>
										<v-list-item >
											<v-list-item v-for="(item, i) in filteredNuclids" :key="i"
												...
												@dragstart="onDragStart(i)" @dragover.prevent draggable="true">
												<!-- {{ i + 1 }} -->
								...
								<div class="bordered mt-2" v-show="!showNuclidsTable">
									<RadioNuclids :codRAO="codRAO" :selected="selected"
									...
									<v-list-item class="bordered pa-10" :style="rightPanel">
										<v-list-item v-for="(item, i) in selectedNuclids" :key="i"
											...
											<div class="calc__nuclids-card">
												...
												<v-text-field label="Удельная активность" :key="UdAKey"
													v-show="showUda === 0" v-model="item.UdA" @change="isCorrect()"
													:rules="[rules.required]" hide-details="auto" suffix="Бк/г"
													hint="Возможен ввод в экспоненциальной форме в формате '1.5+9'"></v-text-field>
			...
			<v-row class="d-flex justify-center pa-6">
				<v-btn :disabled="!enabledBTN" height="50px" width="50%" @click="calcCodRAO">
					Рассчитать
				</v-btn>
				<v-dialog v-model="showNuclidsTable" persistent transition="dialog-bottom-transition" width="auto">
					<v-card>
						<Kod :codRAO="codRAO" :kodRAO="kodRAO" :selectedNuclids="selectedNuclids"
